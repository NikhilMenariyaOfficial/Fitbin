<x-layouts.admin>
    ...
    <div class="card border-0 shadow mt-0">
        <div class="card-header pt-3 pb-2 d-flex">
            ...
            <p class="m-0 bg-transparent"><b>&nbsp;Income Overview</b></p>
        </div>
        <form action="{{ route('overview.income') }}" method="GET" class="card-body pt-3 ps-3 row">
    ...
    <div class="card shadow mt-4">
        <div class="card-header pt-0 pb-2 d-flex justify-content-between">
            <section class="d-flex mt-3">
                <span><i class="fas fa-table me-2"></i></span>
                <p class="m-0 bg-transparent"><b>Incomes From : {{ $fromDate }} To : {{ $toDate }}</b></p>
            </section>
            <section>
                <x-table-limit-button :limit="$limit" route="overview.income"></x-table-limit-button>
            </section>
        </div>
        <div class="card-body border-0 table-wrapper table-responsive">
                ...
                <tbody>
                    @forelse ($payments as $index => $payment)
                        <tr>
                            ...
                            <td>{{ $payment->member->membership_fee }}</td>
                            <td>{{ $payment->amount }}</td>
                            <td>{{ $payment->payment_date }}</td>
                            <td class="rounded-end">{{ $payment->member->membership_end_date }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="text-center"><b>No payment found.</b></td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            <section
                ...
                <div class="fw-normal small mt-4 mt-lg-0">
                    Showing <b>{{ $payments->firstItem() }}</b> out of <b>{{ $payments->total() }}</b> entries
                </div>
            </section>
        </div>
    </div>
